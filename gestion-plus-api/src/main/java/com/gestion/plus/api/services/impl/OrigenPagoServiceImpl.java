...
public class OrigenPagoServiceImpl implements IOrigenPagoService {
  ...
  public ResponseEntity<ResponseDTO> findAll() {
    ...
    try {
      responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Consulta exitosamente").code(Integer.valueOf(HttpStatus.OK.value())).response(OrigenPagoMapper.INSTANCE.beanListToDtoList(this.origenPagoRepository.findAll())).build();
    } catch (Exception e) {
  ...
  public ResponseEntity<ResponseDTO> findOrigenPagoById(Integer id) {
    ...
    if (origenPagoOptional.isPresent()) {
      ...
      responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Consulta exitosamente").code(Integer.valueOf(HttpStatus.OK.value())).response(origenPagoDTO).build();
    } else {
      responseDTO = ResponseDTO.builder().code(Integer.valueOf(HttpStatus.NOT_FOUND.value())).message("Origen de pago no encontrado para el Id: " + id).response(null).build();
    } 
    return ResponseEntity.status(responseDTO.getCode().intValue()).body(responseDTO);
  }
  ...
  public ResponseEntity<ResponseDTO> updateOrigenPago(Integer id, OrigenPagoDTO origenPagoDTO) {
    ...
    if (existingOrigenPago != null) {
      ...
      OrigenPagoDTO origenPagoDTOR = OrigenPagoMapper.INSTANCE.entityToDto((OrigenPagoEntity)this.origenPagoRepository.save(existingOrigenPago));
      responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Actualizado exitosamente").code(Integer.valueOf(HttpStatus.OK.value())).response(origenPagoDTOR).build();
    } else {
  ...
  public ResponseEntity<ResponseDTO> deleteOrigenPago(Integer id) {
    try {
      ...
      ResponseDTO responseDTO = ResponseDTO.builder().success(Boolean.valueOf(false)).message("Error al eliminar, registro no encontrado").code(Integer.valueOf(HttpStatus.NOT_FOUND.value())).response("orgien de pago no encontrado para el Id: " + id).build();
      return ResponseEntity.status(responseDTO.getCode().intValue()).body(responseDTO);
    } 
  }
}
