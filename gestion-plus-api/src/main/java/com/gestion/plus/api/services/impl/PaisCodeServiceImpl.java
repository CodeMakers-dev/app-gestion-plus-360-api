...
public class PaisCodeServiceImpl implements IPaisCodeService{
		...
		try {
			...
			if (paisOptional.isPresent()) {
				...
				responseDTO = ResponseDTO.builder().success(Boolean.valueOf(false))
						.message("pais + codigo no encontrado").code(Integer.valueOf(HttpStatus.NOT_FOUND.value()))
						.response(null).build();
			}
	...
	public ResponseEntity<ResponseDTO> findAll() {
		...
		try {
			responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Consulta exitosamente")
					...
					.response(PaisCodeMapper.INSTANCE.beanListToDtoList(this.paisCodeRepository.findAll()))
					.build();
		} catch (Exception e) {
			responseDTO = ResponseDTO.builder().success(Boolean.valueOf(false)).message("Error al consultar")
					.code(Integer.valueOf(HttpStatus.BAD_REQUEST.value())).response("No se ha encontrado ningun registro")
					.build();
		}
		return ResponseEntity.status(responseDTO.getCode().intValue()).body(responseDTO);
	}
	...
	public ResponseEntity<ResponseDTO> updatePaisCode(Integer id, PaisCodeDTO paisCodeDTO) {
		...
		if (existingPaisCode != null) {
			existingPaisCode.setActivo((paisCodeDTO.getActivo() != null) ? paisCodeDTO.getActivo()
					: existingPaisCode.getActivo());
