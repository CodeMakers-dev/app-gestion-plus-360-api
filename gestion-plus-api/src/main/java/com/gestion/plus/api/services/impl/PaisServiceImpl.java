...
public class PaisServiceImpl implements IPaisService {
  ...
  public ResponseEntity<ResponseDTO> findAll() {
    ...
    try {
      responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Consulta exitosamente").code(Integer.valueOf(HttpStatus.OK.value())).response(PaisMapper.INSTANCE.beanListToDtoList(this.paisRepository.findAll())).build();
    } catch (Exception e) {
  ...
  public ResponseEntity<ResponseDTO> findPaisById(Integer id) {
    ...
    if (paisOptional.isPresent()) {
      ...
      responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Consulta exitosamente").code(Integer.valueOf(HttpStatus.OK.value())).response(paisDTO).build();
    } else {
      responseDTO = ResponseDTO.builder().code(Integer.valueOf(HttpStatus.NOT_FOUND.value())).message("pais no encontrado para el Id: " + id).response(null).build();
    } 
    return ResponseEntity.status(responseDTO.getCode().intValue()).body(responseDTO);
  }
  ...
  public ResponseEntity<ResponseDTO> updatePais(Integer id, PaisDTO paisDTO) {
    ...
    if (existingPais != null) {
      existingPais.setNombre(
          (paisDTO.getNombre() != null) ? paisDTO.getNombre() : existingPais.getNombre());
      existingPais.setCodigoIso((paisDTO.getCodigoIso() != null) ? paisDTO.getCodigoIso() : 
          existingPais.getNombre());
      existingPais.setActivo(
          (paisDTO.getActivo() != null) ? paisDTO.getActivo() : existingPais.getActivo());
      PaisDTO paisDTOR = PaisMapper.INSTANCE.entityToDto((PaisEntity)this.paisRepository.save(existingPais));
      responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Actualizado exitosamente").code(Integer.valueOf(HttpStatus.OK.value())).response(paisDTOR).build();
    } else {
  ...
  public ResponseEntity<ResponseDTO> deletePais(Integer id) {
    try {
      ...
      ResponseDTO responseDTO = ResponseDTO.builder().success(Boolean.valueOf(false)).message("Error al eliminar, registro no encontrado").code(Integer.valueOf(HttpStatus.NOT_FOUND.value())).response("pais no encontrado para el Id: " + id).build();
      return ResponseEntity.status(responseDTO.getCode().intValue()).body(responseDTO);
    } 
  }
}
