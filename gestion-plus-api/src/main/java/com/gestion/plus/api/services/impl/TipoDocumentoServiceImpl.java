...
public class TipoDocumentoServiceImpl implements ITipoDocumentoService {
  ...
  public ResponseEntity<ResponseDTO> saveTipoDocumento(TipoDocumentoDTO tipoDocumentoDTO) {
    ...
    try {
      ...
      TipoDocumentoEntity savedEntity = (TipoDocumentoEntity)this.tipoDocumentoRepository.save(tipoDocumentoEntity);
      TipoDocumentoDTO savedDTO = TipoDocumentoMapper.INSTANCE.entityToDto(savedEntity);
      log.info("Fin mguardar tipo documento");
      ResponseDTO responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Guardado exitosamente").code(Integer.valueOf(HttpStatus.CREATED.value())).response(savedDTO).build();
      return ResponseEntity.status((HttpStatusCode)HttpStatus.CREATED).body(responseDTO);
    } catch (Exception e) {
      ResponseDTO errorResponse = ResponseDTO.builder().success(Boolean.valueOf(false)).message("Error al guardar").code(Integer.valueOf(HttpStatus.BAD_REQUEST.value())).build();
  ...
  public ResponseEntity<ResponseDTO> findAll() {
    ...
    try {
      responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Consulta exitosamente").code(Integer.valueOf(HttpStatus.OK.value())).response(TipoDocumentoMapper.INSTANCE.beanListToDtoList(this.tipoDocumentoRepository.findAll())).build();
    } catch (Exception e) {
  ...
  public ResponseEntity<ResponseDTO> findTipoDocumentoById(Integer id) {
    ...
    if (tipoDocumentoOptional.isPresent()) {
      ...
      responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Consulta exitosamente").code(Integer.valueOf(HttpStatus.OK.value())).response(tipoDocumentoDTO).build();
    } else {
      responseDTO = ResponseDTO.builder().code(Integer.valueOf(HttpStatus.NOT_FOUND.value())).message("tipo de documento no encontrado para el Id: " + id).response(null).build();
    } 
    return ResponseEntity.status(responseDTO.getCode().intValue()).body(responseDTO);
  }
  ...
  public ResponseEntity<ResponseDTO> updateTipoDocumento(Integer id, TipoDocumentoDTO tipoDocumentoDTO) {
    ...
    if (existingTipoDocumento != null) {
      existingTipoDocumento.setNombre(
          (tipoDocumentoDTO.getNombre() != null) ? tipoDocumentoDTO.getNombre() : existingTipoDocumento.getNombre());
      existingTipoDocumento.setDescripcion((tipoDocumentoDTO.getDescripcion() != null) ? tipoDocumentoDTO.getDescripcion() : 
          existingTipoDocumento.getNombre());
      existingTipoDocumento.setActivo(
          (tipoDocumentoDTO.getActivo() != null) ? tipoDocumentoDTO.getActivo() : existingTipoDocumento.getActivo());
      TipoDocumentoDTO tipoDocumentoDTOR = TipoDocumentoMapper.INSTANCE.entityToDto((TipoDocumentoEntity)this.tipoDocumentoRepository.save(existingTipoDocumento));
      responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Actualizado exitosamente").code(Integer.valueOf(HttpStatus.OK.value())).response(tipoDocumentoDTOR).build();
    } else {
      responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Error al actualizar").code(Integer.valueOf(HttpStatus.NOT_FOUND.value())).response(HttpStatus.NOT_FOUND).build();
    } 
    return ResponseEntity.status(responseDTO.getCode().intValue()).body(responseDTO);
  }
  ...
  public ResponseEntity<ResponseDTO> deleteTipoDocumento(Integer id) {
    try {
      ...
      ResponseDTO responseDTO = ResponseDTO.builder().success(Boolean.valueOf(false)).message("Error al eliminar, registro no encontrado").code(Integer.valueOf(HttpStatus.NOT_FOUND.value())).response("Tipo de documento no encontrado para el Id: " + id).build();
      return ResponseEntity.status(responseDTO.getCode().intValue()).body(responseDTO);
    } 
  }
}