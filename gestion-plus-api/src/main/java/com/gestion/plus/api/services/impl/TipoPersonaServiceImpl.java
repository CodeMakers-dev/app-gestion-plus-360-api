...
public class TipoPersonaServiceImpl implements ITipoPersonaService {
  ...
  public ResponseEntity<ResponseDTO> findAll() {
    ...
    try {
      responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Consulta exitosamente").code(Integer.valueOf(HttpStatus.OK.value())).response(TipoPersonaMapper.INSTANCE.beanListToDtoList(this.tipoPersonaRepository.findAll())).build();
    } catch (Exception e) {
  ...
  public ResponseEntity<ResponseDTO> findTipoPersonaById(Integer id) {
    ...
    if (tipoPersonaOptional.isPresent()) {
      ...
      responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Consulta exitosamente").code(Integer.valueOf(HttpStatus.OK.value())).response(tipoPersonaDTO).build();
    } else {
      responseDTO = ResponseDTO.builder().code(Integer.valueOf(HttpStatus.NOT_FOUND.value())).message("tipo persona no encontrado para el Id: " + id).response(null).build();
    } 
    return ResponseEntity.status(responseDTO.getCode().intValue()).body(responseDTO);
  }
  ...
  public ResponseEntity<ResponseDTO> updateTipoPersona(Integer id, TipoPersonaDTO tipoPersonaDTO) {
    ...
    if (existingTipoPersona != null) {
      existingTipoPersona.setNombre(
          (tipoPersonaDTO.getNombre() != null) ? tipoPersonaDTO.getNombre() : existingTipoPersona.getNombre());
      existingTipoPersona.setDescripcion((tipoPersonaDTO.getDescripcion() != null) ? tipoPersonaDTO.getDescripcion() : 
          existingTipoPersona.getNombre());
      existingTipoPersona.setActivo(
          (tipoPersonaDTO.getActivo() != null) ? tipoPersonaDTO.getActivo() : existingTipoPersona.getActivo());
      TipoPersonaDTO tipoPersonaDTOR = TipoPersonaMapper.INSTANCE.entityToDto((TipoPersonaEntity)this.tipoPersonaRepository.save(existingTipoPersona));
      responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Actualizado exitosamente").code(Integer.valueOf(HttpStatus.OK.value())).response(tipoPersonaDTOR).build();
    } else {
  ...
  public ResponseEntity<ResponseDTO> deleteTipoPersona(Integer id) {
    try {
      ...
      ResponseDTO responseDTO = ResponseDTO.builder().success(Boolean.valueOf(false)).message("Error al eliminar, registro no encontrado").code(Integer.valueOf(HttpStatus.NOT_FOUND.value())).response("Tipo persona no encontrado para el Id: " + id).build();
      return ResponseEntity.status(responseDTO.getCode().intValue()).body(responseDTO);
    } 
  }
}
