...
public class TipoPlanServiceImpl implements ITipoPlanService {
  ...
  public ResponseEntity<ResponseDTO> findAll() {
    ...
    try {
      responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Consulta exitosamente").code(Integer.valueOf(HttpStatus.OK.value())).response(TipoPlanMapper.INSTANCE.beanListToDtoList(this.tipoPlanRepository.findAll())).build();
    } catch (Exception e) {
  ...
  public ResponseEntity<ResponseDTO> findTipoPlanById(Integer id) {
    ...
    if (tipoPlanOptional.isPresent()) {
      ...
      responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Consulta exitosamente").code(Integer.valueOf(HttpStatus.OK.value())).response(tipoPlanDTO).build();
    } else {
      responseDTO = ResponseDTO.builder().code(Integer.valueOf(HttpStatus.NOT_FOUND.value())).message("tipo de plan no encontrado para el Id: " + id).response(null).build();
    } 
    return ResponseEntity.status(responseDTO.getCode().intValue()).body(responseDTO);
  }
  ...
  public ResponseEntity<ResponseDTO> updateTipoPlan(Integer id, TipoPlanDTO tipoPlanDTO) {
    ...
    if (existingTipoPlan != null) {
      ...
      TipoPlanDTO tipoPlanDTOR = TipoPlanMapper.INSTANCE.entityToDto((TipoPlanEntity)this.tipoPlanRepository.save(existingTipoPlan));
      responseDTO = ResponseDTO.builder().success(Boolean.valueOf(true)).message("Actualizado exitosamente").code(Integer.valueOf(HttpStatus.OK.value())).response(tipoPlanDTOR).build();
    } else {
  ...
  public ResponseEntity<ResponseDTO> deleteTipoPlan(Integer id) {
    try {
      ...
      ResponseDTO responseDTO = ResponseDTO.builder().success(Boolean.valueOf(false)).message("Error al eliminar, registro no encontrado").code(Integer.valueOf(HttpStatus.NOT_FOUND.value())).response("Tipo de plan no encontrado para el Id: " + id).build();
      return ResponseEntity.status(responseDTO.getCode().intValue()).body(responseDTO);
    } 
  }
}
